    ...
    mnSaveMapCount(0), mbSaveMap(true), mbPaused(false)
{
...
#if ENABLE_LOOP_CLOSURE    
    mptLoopClosing = new thread(&PLVS2::LoopClosing::Run, mpLoopCloser);
#endif
    ...
    {
        // mpLoopCloser -> mpPointCloudMapping
        mpLoopCloser->mSignalGlobalBundleAdjustmentFinished.connectMember(mpPointCloudMapping.get(), &PointCloudMapping::RebuildMap);
        
        // mpLocalMapper -> mpPointCloudMapping
        mpLocalMapper->mSignalVIBAFinished.connectMember(mpPointCloudMapping.get(), &PointCloudMapping::RebuildMap);        
        
        // mpAtlas -> PointCloudAtlas
        auto pPointCloudAtlas = mpPointCloudMapping->GetPointCloudAtlas();
        mpAtlas->mSignalCreateNewMap.connectMember(pPointCloudAtlas.get(), &PointCloudAtlas<PointCloudMapping::PointT>::CreateNewMap);
        mpAtlas->mSignalChangeMap.connectMember(pPointCloudAtlas.get(), &PointCloudAtlas<PointCloudMapping::PointT>::ChangeMap);  
        mpAtlas->mSignalClearMap.connectMember(pPointCloudAtlas.get(), &PointCloudAtlas<PointCloudMapping::PointT>::clearMap);   
        mpAtlas->mSignalClearAtlas.connectMember(pPointCloudAtlas.get(), &PointCloudAtlas<PointCloudMapping::PointT>::clearAtlas);         
        //mpAtlas->mSignalSetMapBad.connectMember(pPointCloudAtlas.get(), &PointCloudAtlas<PointCloudMapping::PointT>::SetMapBad);   
        mpAtlas->mSignalRemoveBadMaps.connectMember(pPointCloudAtlas.get(), &PointCloudAtlas<PointCloudMapping::PointT>::RemoveBadMaps);                  
    }
    /// < Fix verbosity
...
void System::Shutdown()
{
    ...
    if (mbSaveMap) 
    {
        //SaveCurrentMap(mStrMapfile);    
        SaveAtlas(mStrMapfile, FileType::BINARY_FILE);
    }
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    // NOTE: [Luigi] commented for testing
    // if(mSensor==MONOCULAR)
    // {
    ...
    list<PLVS2::KeyFramePtr>::iterator lRit = mpTracker->mlpReferences.begin();
...
bool System::LoadCurrentMap(const string &filename) 
{
    ...
    vector< PLVS2::KeyFramePtr> vpKFS = mpAtlas->GetAllKeyFrames();
